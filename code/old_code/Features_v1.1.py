...
def check_time_distance_OneCoreCPU(t_array:np.ndarray, iter_array:np.ndarray=None,  dt_backward:np.timedelta64=np.timedelta64(-3600, 's'), dt_forward:np.timedelta64=np.timedelta64(3600, 's')):
    ...
    assert isinstance(dt_backward, np.timedelta64), 'The dtype of dt_backward must be np.timedelta64!'
    assert isinstance(dt_forward, np.timedelta64), 'The dtype of dt_forward must be np.timedelta64!'
    count_sum_list = []
        ...
        if not dt_range: # dt_range is not specified. Calculate the total click count.
            ...
            for groupby_features in features_iterable:
                ...
                for name, group in groups: # group should be a Series.
                    result_temp = check_time_distance_parallel(group.values, num_cpus=num_cpus,  split_threshold=10000, dt_backward=dt_backward_seconds, dt_forward=dt_forward_seconds)
