...
'''
def check_time_distance_gpu(session, t_array:np.ndarray, dt_backward:np.int32, dt_forward:np.int32):
...
var_dict = {} # Global variables.

def init_check_time_distance(X):
    # Using a dictionary is not strictly necessary. You can also
    # use global variables.
    var_dict['X'] = X

# @nb.jit will reduce performance.
def check_time_distance(i_start, i_end, dt_backward:np.int32, dt_forward:np.int32):
    # t_array must be already sorted.
    t_array = np.frombuffer(var_dict['X'], dtype='int32')[i_start : i_end + 1]
    dim = t_array.shape[0]
    count_sum_list = []
    for i in range(dim):
...
class FeatureGenerator():
    ...
    def add_click_count_under_given_feature_combination_within_time_range(df:pd.DataFrame, time_column_name, features_iterable, output_dir, dt_range=[], dtype='int32'): # dt_range = [dt_backward, dt_forward](in seconds)
        ...
        if not dt_range: # dt_range is not specified. Calculate the total click count.
            ...
            wait_time = 3 # Wait seconds when worker process in pool exceed max_pool_volumn.
            ...
            for groupby_features in features_iterable:
                ...
                n_groups = len(groups)
                print('\n{} - Number of groups: {}'.format(datetime.now().strftime(fmt), n_groups))
                ...
                # Unpack groups. Save groups data to shared memory.
                print('\n{} - Unpacking groups...'.format(datetime.now().strftime(fmt)))
                '''
                # In this case, list comprehension is slower than for loop.
                all_group_values = [group.values for (name, group) in groups]
                all_group_indices = [group.index.tolist() for (name, group) in groups]
                print('len(all_group_values)', len(all_group_values))
                print('len(all_group_indices)', len(all_group_indices))
                '''
                for name, group in groups:
                    all_group_values.append(group.values)
                    all_group_indices.append(group.index.tolist())
                print('\nlen(all_group_values) = {}'.format(len(all_group_values)))
                print('\nlen(all_group_indices) = {}'.format(len(all_group_indices)))
                print('\n{} - Done.'.format(datetime.now().strftime(fmt)))
                
                print('\n{} - Calculating start and end indices for each group...'.format(datetime.now().strftime(fmt)))
                len_list = [x.shape[0] for x in all_group_values]
                cum_sum = np.cumsum(len_list)
                start_index = [0] + list(cum_sum[0:-1])
                end_index = list(np.array(cum_sum) - 1)
                print('\n{} - Done.'.format(datetime.now().strftime(fmt)))
                
                print('\n{} - Flatterning group values and indices...'.format(datetime.now().strftime(fmt)))
                all_group_values_flat = [item for sublist in all_group_values for item in sublist]
                all_group_indices_flat = [item for sublist in all_group_indices for item in sublist]
                print('\nlen(all_group_values_flat) = {}'.format(len(all_group_values_flat)))
                print('\nlen(all_group_indices_flat) = {}'.format(len(all_group_indices_flat)))
                print('\n{} - Done.'.format(datetime.now().strftime(fmt)))
                
                # Create shared RawArray.
                print('\n{} - Creating shared array for children processes...'.format(datetime.now().strftime(fmt)))
                X = multiprocessing.RawArray('i', len(all_group_values_flat)) # (type, size or array_like)
                # Wrap X as an numpy array so we can easily manipulates its data.
                t_array_shared = np.frombuffer(X, dtype='int32')
                # Copy data to our shared array.
                np.copyto(t_array_shared, all_group_values_flat)
                print('\n{} - Done.'.format(datetime.now().strftime(fmt)))
                
                # Create a multiprocessing pool with safe_lock.
                # I create it within for-loop in order to delet it and release memory when each iteration finish.
                pool = multiprocessing.Pool(processes=num_cpus_use, initializer=init_check_time_distance, initargs=(X,))
                
                for i_start, i_end in zip(start_index, end_index): # group should be a Series.
                    #print('\ni_start = {}, i_end = {}'.format(i_start, i_end))
                    #print('\nt_array = {}'.format(t_array_shared[i_start: i_end+1]))
                    if len(pool._cache) >= max_pool_volumn:
                        print('\n{} - Reached maximium pool volumn (max_pool_volumn = {}), wait for {} seconds...'.format(datetime.now().strftime(fmt), max_pool_volumn, wait_time))
                        time.sleep(wait_time)
                        print('\n{} - processed = {}, in_pool = {}, total = {}'.format(datetime.now().strftime(fmt), len(multiprocessing_results), len(pool._cache), n_groups))
                    
                    multiprocessing_results.append(pool.apply_async(check_time_distance, args=(i_start, i_end, dt_backward_int_seconds, dt_forward_int_seconds)))
                    #multiprocessing_results.append(pool.apply(check_time_distance, args=(i_start, i_end, dt_backward_int_seconds, dt_forward_int_seconds)))
                ...
                counts_in_time_range_df = pd.DataFrame(all_counts_list, index=all_group_indices_flat, columns=[new_feature_name])
                print('\n{} - Done.'.format(datetime.now().strftime(fmt)))
                ...
                try:
                    ...
                    print('\n{} - Memory Error!'.format(datetime.now().strftime(fmt)))
                    print('\n{} - Save counts_in_time_range_df to feather file....format(datetime.now().strftime(fmt))'.format(datetime.now().strftime(fmt)))
                    counts_in_time_range_df.to_feather(os.path.join(output_dir, new_feature_name + '_counts_in_time_range_df.feather'))
                    print('\n{} - Release Memory...'.format(datetime.now().strftime(fmt)))
                    print('\n{} - Objects collected = {}'.format(datetime.now().strftime(fmt), gc.collect()))
                    print('\n{} - Try again...'.format(datetime.now().strftime(fmt)))
                    df_tmp = df.merge(counts_in_time_range_df, left_index=True, right_index=True, how='left')
                ...
                del df_tmp
                print('\n{} - Delete counts_in_time_range_df and df_tmp. Objects collected = {}'.format(datetime.now().strftime(fmt), gc.collect()))        
