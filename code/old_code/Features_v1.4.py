...
class FeatureGenerator():
    ...
    def add_date_time(df:pd.DataFrame, time_column_name:str, output_dir:str):
        ...
        df_year.to_feather(os.path.join(output_dir, 'year.feather'))
        del df_year
        gc.collect()
        
        print('Generating "month" feature...')
        df_month = df[time_column_name].dt.month.astype('int8')
        df_month.to_feather(os.path.join(output_dir, 'month.feather'))
        del df_month
        gc.collect()
        ...
        df_weekday.to_feather(os.path.join(output_dir, 'weekday.feather'))
        del df_weekday
        gc.collect()
        
        print('Generating "day" feature...')
        df_day = df[time_column_name].dt.day.astype('int8')
        df_day.to_feather(os.path.join(output_dir, 'day.feather'))
        del df_day
        gc.collect()
        
        print('Generating "hour" feature...')
        df_hour = df[time_column_name].dt.hour.astype('int8')
        df_hour.to_feather(os.path.join(output_dir, 'hour.feather'))
        def df_hour
        gc.collect()
        
        print('Generating "minute" feature...')
        df_minute = df[time_column_name].dt.minute.astype('int8')
        df_minute.to_feather(os.path.join(output_dir, 'minute.feather'))
        def df_minute
        gc.collect()
        
        print('Generating "second" feature...')
        df_second = df[time_column_name].dt.second.astype('int8')
        df_second.to_feather(os.path.join(output_dir, 'second.feather'))
        def df_second
        gc.collect()
        
        print('Generating "hour_of_day" feature...')
        df_hour_of_day = (df['hour'] + df['minute'] / 60.0 + df['second'] / 3600.0).astype('float32')
        df_hour_of_day.to_feather(os.path.join(output_dir, 'hour_of_day.feather'))
        def df_hour_of_day
        gc.collect()
        
        print('Generating "hour_of_day_sin" feature...')
        df_hour_of_day_sin = (df['hour_of_day'].apply(lambda x: math.sin((x - 12) / 24 * 2 * math.pi))).astype('float32')
        df_hour_of_day_sin.to_feather(os.path.join(output_dir, 'hour_of_day_sin.feather'))
        def df_hour_of_day_sin
        gc.collect()
        
        print('Generating "hour_of_day_cos" feature...')
        df_hour_of_day_cos = (df['hour_of_day'].apply(lambda x: math.cos((x - 12) / 24 * 2 * math.pi))).astype('float32')
        df_hour_of_day_cos.to_feather(os.path.join(output_dir, 'hour_of_day_cos.feather'))
        def df_hour_of_day_cos
        gc.collect()
    ...
    def add_click_time_delta_under_given_feature_combination(df:pd.DataFrame, time_column_name, features_iterable, output_dir, direction=None, dtype='float32'):
        ...
        for groupby_features in features_iterable:
            ...
            if direction == 'forward':
                ...
                df_tmp.to_feather(os.path.join(output_dir, new_feature_name + '.feather'))
                del df_tmp
                gc.collect()
                ...
                df_tmp_forward.to_feather(os.path.join(output_dir, new_feature_name_forward + '.feather'))
                del df_tmp_forward
                gc.collect()
                ...
                df_tmp_backward.to_feather(os.path.join(output_dir, new_feature_name_backward + '.feather'))
                del df_tmp_backward
                gc.collect()
        return df
    ...
    def add_click_count_under_given_feature_combination(df:pd.DataFrame, features_iterable, output_dir):
        ...
        for groupby_features in features_iterable:
            ...
            df_tmp[new_feature_name].to_feather(os.path.join(output_dir, new_feature_name + '.feather'))
            # Clean memory.
    ...
    def add_click_count_under_given_feature_combination_within_time_range(df:pd.DataFrame, time_column_name,  features_iterable, dt_range=[]): # dt_range = [dt_backward, dt_forward](in seconds)
        ...
        if not dt_range: # dt_range is not specified. Calculate the total click count.
            ...
            for one_result in multiprocessing_results:
                ...
                df[new_feature_name].to_feather(os.path.join(output_dir, new_feature_name + '.feather'))
            ...
            for groupby_features in features_iterable:
                ...
                for name, group in groups: # group should be a Series.
                    result_temp = check_time_distance_parallel_jit(group.values, num_cpus=num_cpus,  split_threshold=10000, dt_backward=dt_backward_int_seconds, dt_forward=dt_forward_int_seconds)
    ...
    def add_unique_count_of_feature_under_given_feature_combination(df:pd.DataFrame, features_iterable, objective_features:list):
        ...
        for groupby_features in features_iterable:
            ...
            df_tmp.to_feather(os.path.join(output_dir, rename_dict[feature] + '.feather'))
            # Clean memory.
            del all_features
            del rename_dict
