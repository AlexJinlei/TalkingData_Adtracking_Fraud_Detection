# Features_v3.4.py
# Restart pool every 1e6 task workers.
# Use share memory with father process.
# It turns out to be that when pool is restarted each time, the usage of memory of main (father) process will increase. 
...
var_dict = {} # Global variables for shared memory.
...
class FeatureGenerator():
    ...
    def add_click_count_under_given_feature_combination_within_time_range(df:pd.DataFrame, time_column_name, features_iterable, output_dir, dt_range=[], dtype='int32'): # dt_range = [dt_backward, dt_forward](in seconds)
        ...
        if not dt_range: # dt_range is not specified. Calculate the total click count.
            ...
            pool_volume = int(1e6) # Restart pool when reach pool_volume.
            ...
            for groupby_features in features_iterable:
                ...
                for i, i_start, i_end in zip(range(len(start_index)), start_index, end_index): # group should be a Series.
                    if (not (i % 10000)) & (i > 0):
                        print('\n{} - {}/{} worker processes added.'.format(datetime.now().strftime(fmt), i, n_groups))
                    
                    if (not (i % pool_volume)) & (i > 0):
                        print('\n{} - i = {}. Reached maximum pool volume ({}), restarting pool...'.format(datetime.now().strftime(fmt), i, pool_volume))
                        ...
                        # Restarting pool.
                        pool = multiprocessing.Pool(processes=num_cpus_use, initializer=init_check_time_distance, initargs=(X,))
                        print('\n{} - Pool is restarted.'.format(datetime.now().strftime(fmt)))
