...
def check_time_distance_OneCoreCPU(t_array:np.ndarray, iter_array:np.ndarray=None,  dt_backward:np.timedelta64=np.timedelta64(-3600, 's'), dt_forward:np.timedelta64=np.timedelta64(3600, 's')):
...
def check_time_distance_parallel(t_array:np.ndarray, iter_array:np.ndarray=None, num_cpus=2, split_threshold=10000, dt_backward:np.timedelta64=np.timedelta64(-3600, 's'), dt_forward:np.timedelta64=np.timedelta64(3600, 's')):
    if (t_array.shape[0] < split_threshold) | (t_array.shape[0] < num_cpus):
        count_sum_list = check_time_distance_OneCoreCPU(t_array, iter_array=iter_array, dt_backward=dt_backward, dt_forward=dt_forward)
    else:
        # Split input into shares that equals to num_cpus.
        input_list = np.array_split(t_array, num_cpus) # As iter_array
        cell_func = partial(check_time_distance_OneCoreCPU, t_array, dt_backward=dt_backward, dt_forward=dt_forward)
...
class FeatureGenerator():
    ...
    @staticmethod
    def add_click_count_under_given_feature_combination_within_time_range(df:pd.DataFrame, time_column_name,  features_iterable, dt_range=[]): # dt_range = [dt_backward, dt_forward](in minutes)
        ...
        if not dt_range: # dt_range is not specified. Calculate the total click count.
            ...
            for groupby_features in features_iterable:
                ...
                keywords = {'num_cpus': num_cpus, 'split_threshold': 10000,  'dt_backward': dt_backward_seconds, 'dt_forward': dt_forward_seconds}
                for name, group in groups: # group should be a Series.
                    result_temp = pool.apply_async(check_time_distance_parallel, (group.values,), keywords)
                    multiprocessing_results.append(result_temp)
...
df_train_original = pd.read_feather('./data/input/train.feather.small')
...
t0 = datetime.now()
df = FeatureGenerator.add_click_count_under_given_feature_combination_within_time_range(df, 'click_time',  combined_feature, dt_range=[-60, 60]) # dt_range in second.
t1 = datetime.now()
